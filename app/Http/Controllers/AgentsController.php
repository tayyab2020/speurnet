            ...
            if($address)
            {
                if($radius != 0)
                {
                    foreach ($agents->get() as $index => $key)
                    {
                        ...
                        $data = file_get_contents("https://maps.googleapis.com/maps/api/distancematrix/json?key=AIzaSyCa2tNnLK8OjdGb0jlMrKyll2wZnTDLsUk&origins=$from&destinations=$to&language=en-EN&sensor=false");
