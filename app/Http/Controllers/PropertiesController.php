        ...
        if($preferred_address && $preferred_address_latitude && $preferred_address_longitude) {
            ...
            if($preferred_radius != 0) {
                ...
                foreach ($properties->get() as $key) {
                    ...
                    $url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=" . urlencode($preferred_address_latitude) . "," . urlencode($preferred_address_longitude) . "&destinations=" . urlencode($property_latitude) . "," . urlencode($property_longitude) . "&key=AIzaSyCL20x57Ekftp0ZvZKImByPhZe7Tsb-G0E";
                ...
                if($properties) {
                    ...
                    if ($address && $address_latitude && $address_longitude) {
                            ...
                            foreach ($properties as $key) {
                                ...
                                $url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=" . urlencode($address_latitude) . "," . urlencode($address_longitude) . "&destinations=" . urlencode($property_latitude) . "," . urlencode($property_longitude) . "&key=AIzaSyCL20x57Ekftp0ZvZKImByPhZe7Tsb-G0E";
